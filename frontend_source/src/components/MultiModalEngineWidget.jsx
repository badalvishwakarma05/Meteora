...
export default function MultiModalEngineWidget() {
  ...
  return (
    <div className="w-full rounded-2xl bg-black border border-white text-white shadow-2xl p-4 sm:p-5 space-y-4 font-sans relative overflow-hidden">
      {/* HEADER SECTION */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/20 pb-3.5">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-black border border-white flex items-center justify-center text-white flex-shrink-0">
            <Cpu size={18} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-white text-black border border-white uppercase">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto font-mono text-xs">
          <button
            ...
            disabled={loading}
            className="px-3 py-1.5 rounded border border-white bg-black hover:bg-white hover:text-black transition-colors flex items-center gap-1.5 text-xs font-mono cursor-pointer"
      ...
      {/* LIVE PREDICTION HIGHLIGHT CARD (DARK & WHITE MONOCHROME HERO CARD) */}
      <div className="rounded-xl bg-black border border-white p-4 font-mono space-y-3 shadow-inner hover:border-zinc-300 transition-colors">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 border-b border-white/20 pb-3">
          <div className="space-y-1">
            ...
            <div className="text-xs text-zinc-300">
              Satellite Source: <span className="text-white underline decoration-white/40">{satelliteFile}</span>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded text-xs font-bold uppercase tracking-wider bg-white text-black border border-white flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center pt-1">
          ...
          <div className="md:col-span-4 border-r border-white/20 pr-3 space-y-1.5">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-zinc-400 font-bold uppercase">Probability</span>
            </div>
            {/* Crisp 1px White Progress Bar */}
            <div className="w-full h-2 rounded-full bg-zinc-900 border border-white/40 overflow-hidden">
              <div
                className="h-full bg-white transition-all duration-700"
                style={{ width: `${Math.min(100, Math.max(5, prob))}%` }}
              />
            </div>
            <div className="text-[10px] text-zinc-400 flex justify-between pt-0.5">
              <span>0% Stable</span>
              <span className="text-white font-bold">Category 3+ Escalation</span>
              <span>100% Extreme</span>
            </div>
          </div>

          {/* Real-time Atmospheric Precursor Metrics */}
          <div className="md:col-span-4 space-y-2 border-r border-white/20 pr-3">
            <div className="text-[10px] text-zinc-400 uppercase tracking-widest">
              SYNOPTIC PRECURSORS (72h &rarr; 24h &rarr; NOW)
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="p-2 rounded bg-zinc-950 border border-white/20 space-y-0.5">
                <div className="text-[10px] text-zinc-400">CENTRAL PRESSURE</div>
                <div className="text-base font-bold text-white">{pressure} hPa</div>
                <div className="text-[9px] text-zinc-400">&Delta;P: -30 hPa drop</div>
              </div>
              <div className="p-2 rounded bg-zinc-950 border border-white/20 space-y-0.5">
                <div className="text-[10px] text-zinc-400">MAX SUSTAINED WIND</div>
                <div className="text-base font-bold text-white">{windKmh} km/h</div>
                <div className="text-[9px] text-zinc-400">Squally Gale Force</div>
              </div>
            </div>
          ...
          <div className="md:col-span-4 space-y-2">
            ...
            <div className="p-2.5 rounded bg-zinc-950 border border-white text-xs text-white leading-relaxed font-sans font-medium">
              {geoAlert}
            </div>
            <div className="text-[10px] text-zinc-400 flex items-center justify-between font-mono">
              <span>Model: <strong className="text-white">CNN-LSTM v1.0</strong></span>
              <span className="text-white font-bold">&check; SQLite DB Sync Active</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 font-mono">
        ...
        <div className="p-3.5 rounded-xl bg-black border border-white flex flex-col justify-between hover:bg-zinc-950 transition-colors">
          <div className="flex items-center justify-between text-zinc-400 text-xs uppercase tracking-wider mb-2">
            <span>TOTAL SYNCED PRECURSOR RECORDS</span>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-400">Rows Aligned</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-2 border-t border-white/10 pt-2 flex items-center justify-between">
            <span>72h, 48h, 24h Windows</span>
            <span className="text-white font-bold">Balanced Set</span>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-black border border-white flex flex-col justify-between hover:bg-zinc-950 transition-colors">
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-zinc-400">
              + Relative Humidity &amp; Spatial Coordinates
            </div>
          </div>
          <div className="text-[11px] text-zinc-400 mt-2 border-t border-white/10 pt-2 flex items-center justify-between">
            <span>Multi-Modal Embedding: 192-D</span>
        ...
        <div className="p-3.5 rounded-xl bg-black border border-white flex flex-col justify-between hover:bg-zinc-950 transition-colors">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-400">HDF5 Volume Arrays</span>
          </div>
          <div className="text-[11px] text-zinc-400 mt-2 border-t border-white/10 pt-2 flex items-center justify-between">
            <span>Spatial CNN Tensor: 1x128x128</span>
            <span className="text-white font-bold">MOSDAC TIR1</span>
          </div>
        </div>
      </div>

      {/* MINIMALIST TERMINAL-STYLE SCROLLING DATA-FEED COMPONENT */}
      <div className="rounded-xl bg-black border border-white overflow-hidden font-mono text-xs shadow-inner">
        {/* Terminal Title Bar */}
        <div className="flex items-center justify-between px-3.5 py-2.5 bg-black border-b border-white/20">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-400 uppercase tracking-widest font-bold">STREAM ACTIVE</span>
          </div>
        </div>

        {/* Terminal Header Row */}
        <div className="grid grid-cols-12 gap-2 px-3.5 py-2 border-b border-white/20 bg-zinc-950 text-zinc-400 text-[11px] font-bold uppercase tracking-wider">
        ...
        <div className="divide-y divide-white/10 max-h-40 overflow-y-auto bg-black scrollbar-thin scrollbar-thumb-white/20">
          {records.length > 0 ? (
            records.map((row, idx) => (
              <div
                key={idx}
                className="grid grid-cols-12 gap-2 px-3.5 py-2.5 hover:bg-zinc-900/80 transition-colors text-white items-center group text-xs"
              >
                <div className="col-span-3 sm:col-span-2 font-bold text-white flex items-center gap-1 truncate">
                  <span className="text-zinc-500 text-[10px] group-hover:text-white transition-colors">&gt;</span>
                  <span>{row.time || 'N/A'}</span>
                </div>
                <div className="col-span-3 sm:col-span-2 text-zinc-300 font-semibold truncate">
                ...
                <div className="col-span-3 sm:col-span-2 text-right font-mono text-zinc-300 hidden sm:block">
                  {row.surface_pressure ? `${row.surface_pressure} hPa` : '--'}
                </div>
                <div className="col-span-3 sm:col-span-4 text-zinc-400 text-[11px] truncate font-mono group-hover:text-white transition-colors" title={row.satellite_image_path}>
                  {row.satellite_image_path || 'data/MOSDAC_STD_L1B.h5'}
                </div>
              </div>
            ))
          ) : (
            <div className="px-4 py-6 text-center text-zinc-500 font-mono text-xs">
        ...
        <div className="px-3.5 py-2 bg-black border-t border-white/20 text-[10px] text-zinc-400 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1">
          <div className="flex items-center gap-2">
