  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs text-[#88a0c0]">Automated Dvorak & Pattern Recognition</div>
          <h1 className="text-xl font-bold text-white tracking-tight">AI Classification & Analysis</h1>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {/* HDF5 Dataset Selector */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#1a3a6b] bg-[#0d1f3c]">
            ...
            <select
              ...
              className="text-xs bg-[#0a1628] text-white font-mono font-semibold px-2 py-1 rounded border border-[#1a3a6b] focus:outline-none focus:border-[#00d4ff]"
            >
              {availableFiles.length > 0 ? (
                availableFiles.map(f => <option key={f} value={f}>{f}</option>)
              ) : (
                <option value="">No .h5 files found</option>
              )}
            </select>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs sm:text-sm px-3.5 py-2 rounded-lg border border-[#1a3a6b] bg-[#0d1f3c] font-semibold text-[#88a0c0] hover:text-white hover:border-cyan-500/40 transition-all"
          >
            <RefreshCw size={14} className="text-[#00d4ff]" />
            <span>Batch Process</span>
          </button>
          <button
            onClick={handleExportReport}
            className="flex items-center gap-2 text-xs sm:text-sm px-4 py-2 rounded-lg font-bold text-[#050d1a] bg-[#00d4ff] hover:bg-cyan-300 transition-all shadow-lg shadow-cyan-500/20"
          >
            <FileText size={14} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Satellite Viewer */}
        <div className="rounded-xl border border-[#1a3a6b] bg-[#0d1f3c] overflow-hidden flex flex-col">
          {/* Toolbar */}
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1a3a6b] bg-[#0a1628]">
              ...
              {['IR', 'Visible', 'WV', 'RGB'].map(b => (
                <button
                  ...
                  className={`text-xs px-2.5 py-1 rounded font-semibold transition-colors ${
                    band === b ? 'bg-cyan-500/20 text-[#00d4ff] border border-cyan-500/50' : 'bg-[#0d1f3c] text-[#88a0c0] hover:text-white'
                  }`}
                >
                  {b}
                </button>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="w-7 h-7 rounded text-xs font-bold flex items-center justify-center bg-[#0a1628] border border-[#1a3a6b] text-[#88a0c0] hover:text-white hover:border-cyan-500/40 transition-colors"
                title="Zoom In"
              >
                <ZoomIn size={13} />
              </button>
          ...
          >
            ...
            {/* Temperature Bar */}
            <div className="absolute right-3 top-4 bottom-10 w-3 rounded border border-[#1a3a6b] overflow-hidden bg-gradient-to-b from-[#ff1744] via-[#ffd600] to-[#0a1628]" />
          </div>

          {/* Metadata Bar */}
          <div className="px-3.5 py-2 text-xs font-mono flex items-center justify-between bg-[#0a1628] border-t border-[#1a3a6b] text-[#88a0c0]">
            <span className="text-white font-semibold">INSAT-3D · Ch: {band}</span>
            <span>14-Sep 12:00 UTC · Res: 4km</span>
          </div>
        </div>

        {/* Classification Results Panel */}
        <div className="rounded-xl border border-[#1a3a6b] bg-[#0d1f3c] p-5 flex flex-col gap-4">
          {/* Classification Badge */}
          <div
            className="rounded-xl p-3.5 text-center border border-red-500/40 bg-gradient-to-r from-red-500/20 to-orange-500/20 shadow-lg"
          >
            <div className="text-[10px] font-bold tracking-widest text-red-400 uppercase mb-1 font-mono">
              AUTOMATED DVORAK CLASSIFICATION
            </div>
            <div className="text-xl font-black text-white tracking-tight">VERY SEVERE CYCLONIC STORM</div>
            <div className="text-xs font-bold text-red-300 mt-0.5 font-mono">Saffir-Simpson Equivalent: Category 3</div>
          </div>

          {/* AI Confidence Gauge */}
          <div className="flex items-center gap-4 p-3 rounded-lg border border-[#1a3a6b] bg-[#0a1628]">
            <ConfidenceGauge value={94} />
            <div>
              <div className="text-xs font-bold text-[#88a0c0] uppercase tracking-wide">AI Confidence Index</div>
              <div className="text-2xl font-black font-mono text-[#00d4ff]">94.2%</div>
              <div className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1 mt-0.5">
                <CheckCircle size={12} className="text-emerald-400" /> High Certainty · CI 5.5
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5 flex-1">
            <div className="text-xs font-bold tracking-widest uppercase text-[#88a0c0]">
              Extracted Morphological Features
            </div>
            {classificationFeatures.map(f => (
              <div
                key={f.feature}
                className="flex items-center justify-between py-1.5 border-b border-[#1a3a6b]/80 text-xs"
              >
                <span className="text-[#88a0c0]">{f.feature}</span>
                ...
                <span className="text-emerald-400 flex items-center gap-1 text-[11px] font-medium">
                  <CheckCircle size={11} className="text-emerald-400" /> {f.status}
                </span>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={handleExportReport}
            className="w-full py-2.5 rounded-lg font-bold text-xs text-[#050d1a] bg-[#00d4ff] hover:bg-cyan-300 transition-all shadow-md shadow-cyan-500/20"
          >
      ...
      <div className="rounded-xl border border-[#1a3a6b] bg-[#0d1f3c] p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-[#00d4ff] font-semibold font-mono">↑ Continuous Intensification Observed</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {timelineFrames.map((f, i) => (
            <div
              key={i}
              className="rounded-lg overflow-hidden border border-[#1a3a6b] p-2 bg-[#0a1628] hover:border-cyan-500/50 transition-all cursor-pointer"
              onClick={() => showToast(`Selected frame at ${f.time}: Classified as ${f.class}.`, 'info')}
            >
              <div className="h-16 rounded bg-[#050d1a] flex items-center justify-center relative mb-2 border border-[#1a3a6b]">
                <div className="w-8 h-8 rounded-full bg-cyan-500/10 border border-cyan-500/40 cyclone-spin" />
                <span className="absolute bottom-1 right-1 text-[9px] font-mono text-[#88a0c0]">{f.time}</span>
              </div>
              <div className="text-[11px] font-bold text-white truncate">{f.class}</div>
              <div className="text-[10px] font-mono text-[#88a0c0]">Conf: <span className="text-[#00d4ff] font-bold">{f.conf}%</span></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl border border-[#1a3a6b] bg-[#0d1f3c] p-4">
          ...
          <div className="space-y-2.5">
            {historicalCyclones.map((h, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-2.5 rounded-lg border border-[#1a3a6b] bg-[#0a1628] hover:border-cyan-500/40 transition-colors"
              >
                <span className="text-xs font-mono font-bold text-[#88a0c0] w-4">0{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-white">
                    {h.name} <span className="text-[#88a0c0] font-normal">({h.year})</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded">{h.similarity}% Match</span>
                  <button
                    onClick={() => setSelectedHistorical(h)}
                    className="px-2.5 py-1 rounded text-xs font-semibold border border-[#1a3a6b] text-[#88a0c0] hover:text-white hover:border-cyan-500/50 transition-colors"
                  >
                    Learn
                  </button>
                </div>
        ...
        {/* Radar Chart */}
        <div className="rounded-xl border border-[#1a3a6b] bg-[#0d1f3c] p-4 flex flex-col">
          <h3 className="text-xs font-bold tracking-widest text-[#88a0c0] uppercase font-mono mb-3">
          ...
          <div className="flex-1 min-h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
                <PolarGrid stroke="#1a3a6b" />
                <PolarAngleAxis dataKey="axis" tick={{ fill: '#88a0c0', fontSize: 10 }} />
                <Radar name="Active Storm (DANA)" dataKey="current" stroke="#00d4ff" fill="#00d4ff" fillOpacity={0.25} />
                <Radar name="Historical Average" dataKey="historical" stroke="#ff9500" fill="#ff9500" fillOpacity={0.15} />
                <Tooltip
                  contentStyle={{ background: '#0a1628', borderColor: '#1a3a6b', color: '#ffffff', fontSize: 11 }}
      ...
      {batchModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-[#1a3a6b] p-6 bg-[#0d1f3c] shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-white text-sm">Batch Pattern Classification Engine</h3>
              {batchProgress === 100 && (
                <button onClick={() => setBatchModalOpen(false)} className="text-[#88a0c0] hover:text-white">
                  <X size={16} />
                </button>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs text-[#88a0c0]">
                <span>Processing 48 Satellite Frames</span>
                <span className="text-[#00d4ff] font-mono font-bold">{batchProgress}%</span>
              </div>
              <div className="w-full h-2 rounded-full bg-[#0a1628] border border-[#1a3a6b] overflow-hidden">
                <div
            ...
            {batchProgress === 100 && (
              <button
                ...
                className="w-full mt-5 py-2 rounded-lg font-bold text-xs text-[#050d1a] bg-[#00d4ff] hover:bg-cyan-300 transition-colors"
              >
                Close & Return
              </button>
            )}
          ...
          <div className="w-full max-w-lg rounded-xl border border-[#1a3a6b] p-6 bg-[#0d1f3c] shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-[#1a3a6b] mb-4">
              <div>
                ...
                <div className="text-xs text-[#88a0c0] font-mono mt-0.5">
                  {selectedHistorical.similarity}% Structural Similarity to Cyclone DANA
                </div>
              </div>
              <button onClick={() => setSelectedHistorical(null)} className="text-[#88a0c0] hover:text-white">
                <X size={18} />
              </button>
            </div>

            <div className="grid grid-cols-2 gap-3 text-xs mb-4">
              <div className="p-3 rounded border border-[#1a3a6b] bg-[#0a1628]">
                <div className="text-[#88a0c0] text-[10px] font-mono">PEAK SUSTAINED WIND</div>
                <div className="font-mono font-bold text-base text-white mt-0.5">{selectedHistorical.maxWind} km/h</div>
              </div>
              <div className="p-3 rounded border border-[#1a3a6b] bg-[#0a1628]">
                <div className="text-[#88a0c0] text-[10px] font-mono">LANDFALL ZONE</div>
                <div className="font-bold text-base text-white mt-0.5">{selectedHistorical.landfall}</div>
              </div>
            </div>

            <p className="text-xs text-[#88a0c0] leading-relaxed mb-5">
